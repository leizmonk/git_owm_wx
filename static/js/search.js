$(function() {
    ...
    function wxSearch() {
        ...
        $.getJSON(queryURL, function(data) {
            ...
            if(data.weather[0].id == '800') { 
                $('body').css({
                    'background': 'url("/static/weather/imgs/clear-sky.jpg") no-repeat center center fixed',
                });
            } else if(data.weather[0].id == '801') {
                $('body').css({
                    'background': 'url("/static/weather/imgs/few-clouds.jpg") no-repeat center center fixed',
                });
            } else if(data.weather[0].id == '802') {
                $('body').css({
                    'background': 'url("/static/weather/imgs/broken-clouds.jpg") no-repeat center center fixed',
                });
                ...
                $('body').css({
                    'background': 'url("/static/weather/imgs/overcast.jpg") no-repeat center center fixed',
                });
            } else if($.inArray(data.weather[0].id, ['300','301','302','310','311','312','313','314','321','500','520'])) {
                $('body').css({
                    'background': 'url("/static/weather/imgs/rain-lite.jpg") no-repeat center center fixed',
                });
            } else if(data.weather[0].id == '501' || data.weather[0].id == '521') {
                $('body').css({
                    'background': 'url("/static/weather/imgs/rain-med.jpg") no-repeat center center fixed',
                });
            } else if(data.weather[0].id == '600' || data.weather[0].id == '620') {
                $('body').css({
                    'background': 'url("/static/weather/imgs/light-snow.jpg") no-repeat center center fixed',
                });
            } else if($.inArray(data.weather[0].id, ['601','602','615','616','621','622'])) {
                $('body').css({
                    'background': 'url("/static/weather/imgs/heavy-snow.jpg") no-repeat center center fixed',
                });
            } else if($.inArray(data.weather[0].id, ['511','611','612'])) {
                $('body').css({
                    'background': 'url("/static/weather/imgs/freezing-rain.jpg") no-repeat center center fixed',
                });  
            } else if(data.weather[0].id == '701' || data.weather[0].id == '741') {
                $('body').css({
                    'background': 'url("/static/weather/imgs/dense-fog.jpg") no-repeat center center fixed',
                });
            } else if(data.weather[0].id == '711') {
                $('body').css({
                    'background': 'url("/static/weather/imgs/smoky-sky.jpg") no-repeat center center fixed',
                });
            } else if(data.weather[0].id == '721') {
                $('body').css({
                    'background': 'url("/static/weather/imgs/haze.jpg") no-repeat center center fixed',
                });
            } else if($.inArray(data.weather[0].id, ['731','751','761'])) {
                $('body').css({
                    'background': 'url("/static/weather/imgs/sand.jpg") no-repeat center center fixed',
                });
            } else if($.inArray(data.weather[0].id, ['502','503','504','522'])) {
                $('body').css({
                    'background': 'url("/static/weather/imgs/heavy_rain.jpg") no-repeat center center fixed',
                });
            } else if($.inArray(data.weather[0].id, ['200','201','202','210','211','212','221','230','231','232'])) {
                $('body').css({
                    'background': 'url("/static/weather/imgs/thunderstorm.jpg") no-repeat center center fixed',
                });
            } else {
                return false;
            }
    ...
    function convertWindDirection(data) {
        ...
        var val = Math.round((((degrees/22.5) + .5)) % 16);
        var directions = [
            'North', 
            'North-Northeast', 
            'Northeast', 
